...
LIST_target = [BTC_0_dic, ETH_1_dic, WAVES_7_dic, STEEM_11_dic, XRP_4_dic, ETC_5_dic, BTG_20_dic]
...
while True:
    try:
        ...
        if (0 < (now.minute % time_unit) <= 31) & (0 < (now.second % 60) <= 59):  # N시:01:00초 ~ N시:01:59초 사이 시각이면
        ...
        for k in range(0, len(LIST_target), 1):
            ...
            if LIST_target[k]['bought_state'] == 1:
                ...
                print('(pyupbit.get_current_price(coin_inv_2) / get_avg_buy_price(coin_inv_2[4:])) < (1 - LIST_target[k][ratio_sell_forced]) ____ {0} < {1}'.                      format((pyupbit.get_current_price(coin_inv_2) / get_avg_buy_price(coin_inv_2)), (1 - LIST_target[k]['ratio_sell_forced'])))
